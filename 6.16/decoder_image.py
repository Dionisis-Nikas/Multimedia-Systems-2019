...
def quantization(int1, quant_array):
    ...
    draw.text(xy=(120, 0), text="Quantize Image", fill=(255, 69, 0), font=type_font)
...
def de_quantization(int2, recover_img):
    ...
    # print(recover_img)
    # print('\n\n\n\n Recover from quantization\n', recover_img)
    im = Image.fromarray(recover_img)

    type_font = ImageFont.truetype("arial.ttf", 20)
    draw = ImageDraw.Draw(im)
    draw.text(xy=(80, 0), text="De-Quantize Image", fill=(255, 69, 0), font=type_font)
...
def decoder(encoded):
    ...
    for i in encoded_array:
        ...
        # check if there are still pixel to check  (?)
        while present < end:
...
draw.text(xy=(120, 0), text="Original Image", fill=(255, 69, 0), font=type_font)
...
draw.text(xy=(90, 0), text="Create Image from text", fill=(255, 69, 0), font=type_font)

img.show()

with open('apotelesmata_11.txt', 'w') as text:
